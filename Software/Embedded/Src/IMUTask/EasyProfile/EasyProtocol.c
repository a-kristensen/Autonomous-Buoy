...
#       endif
        ...
        HEAD_2_          = 0x55;  totalMem += sizeof(HEAD_2_);          // This value is not used if HEAD_LENGTH==1
...
int EasyProtocol_Init(
   ...
   // outBuf    = (char*)calloc((oDS+EP_PKG_MODIFIER_SIZE_), sizeof(char)); totalMem += (oDS+EP_PKG_MODIFIER_SIZE_);
    ...
    // added because calloc() do not work!!!
    int size_inBuf = (iDS+EP_PKG_MODIFIER_SIZE_)* sizeof(char);
    int size_outBuf = (oDS+EP_PKG_MODIFIER_SIZE_)* sizeof(char);
    void* mem_inBuf = pvPortMalloc(size_inBuf);
    void* mem_outBuf = pvPortMalloc(size_outBuf);
    p = inBuf = memset(mem_inBuf,0,size_inBuf);
    outBuf = memset(mem_outBuf,0,size_outBuf);

    //p = inBuf = (char*)pvPortMalloc((iDS+EP_PKG_MODIFIER_SIZE_)* sizeof(char)); totalMem += (iDS+EP_PKG_MODIFIER_SIZE_);
    //outBuf    = (char*)pvPortMalloc((oDS+EP_PKG_MODIFIER_SIZE_)* sizeof(char)); totalMem += (oDS+EP_PKG_MODIFIER_SIZE_);
...
#   endif
    ...
    if(CHECKSUM_LENGTH_ == 1) {
        //recvChecksum = packageData[payloadSize + EP_PKG_MARKUP_SIZE_- CHECKSUM_LENGTH_ - roundUp];
        recvChecksum = packageData[payloadSize + EP_PKG_MODIFIER_SIZE_- CHECKSUM_LENGTH_];
    }
    else if(CHECKSUM_LENGTH_ == 2){
        //recvChecksum = *((int16*)(packageData + payloadSize + EP_PKG_MARKUP_SIZE_ - CHECKSUM_LENGTH_ - roundUp));
